function plot_countryprojection_icc(country)
...
switch country
    case 'guinea'
[p,s,cumdata,cumdata_all,inc_data,inc_all,xsupport] = fitcasedata_pois_data(country);
...
theoryfit = @(x) inc_data(end).*exp(p(2).*x);%.*(exp(p(2))-1);
theory1 = @(x) inc_data(end).*exp(mlambda1.*x);%.*(exp(mlambda1)-1);
theory2 = @(x) inc_data(end).*exp(mlambda2.*x);%.*(exp(mlambda2)-1);
...
% figure is drawn
eval(strcat('tmpfilename = ''figures/figcountry_dataproj_',country,'_icc'';'))
...
tmph = plot(xtimes,inc_data,'k.');
set(tmph,'linewidth',3,'MarkerSize',30);

tmph = plot(xtimes(end)+xtimesextend(2:end),inc_all(numel(xtimes)+1:numel(xtimes)+numel(xtimesextend)-1),'gd','MarkerFaceColor','g');
...
ylabel('Incident case count','fontsize',20,'verticalalignment','bottom','interpreter','latex');
