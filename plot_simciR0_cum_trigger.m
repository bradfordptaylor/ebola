...
clear all
close all
%plot sims using cumulative data- R0 CI
rhod = .25;
...
load(filenameload)
loadname = strcat('data/xsolve_simci_cum');
load(loadname)
loadname = strcat('data/lambsolve_simci_cum');
load(loadname)
loadname = strcat('data/interpvec_simci_cum');
load(loadname)




real_lambda = 1/20;  % Exponential growth rate as fit to the data
...
eval(strcat('tmpfilename = ''figures/fig_simciR0_cum_large'';'))
...
hold on
% tmph = plot(interpvec(1,:),R0(interpvec(1,:).^-1),'b-');
% set(tmph,'linewidth',3);
% tmph = plot(interpvec(2,:),R0(interpvec(2,:).^-1),'b-');
% set(tmph,'linewidth',3);

% tmph = plot(data.mlambda.^-1,R0(data.mlambda),'bo-');
% set(tmph,'linewidth',3);
...
set(tmph,'linewidth',3);
% tmph=plot(data.lambda_low.^(-1),R0(data.lambda_low),'k.');
% set(tmph,'MarkerSize',30);
% tmph=plot(data.lambda_high.^(-1),R0(data.lambda_high),'k.');
set(tmph,'MarkerSize',30);
tmph=plot(interpvec(2,:),R0(lambsolve(:,2)'.^-1),'k-');    %bottom CI
set(tmph,'linewidth',3,'MarkerSize',30);
 
ROmed = R0(real_lambda)
R0CI = [R0(pt1^(-1)) R0(pt2^(-1))]
